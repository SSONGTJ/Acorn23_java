...
<%
	...
	// EL + JSTL 을 테스트 하기 위해서 응답에 필요한 데이터 (model)를 request 영역에 담기
	// "list" 라는 키 값으로 파일목록(List<FileDto>) 담기
			...
			<tbody>
				<c:forEach var="tmp" items="${requestScope.list }">
					<tr>
						...
						<td>${tmp.regdate }</td>
						<td>
							<c:if test="${tmp.num eq id }">
								<a href="protected/delete.jsp?num=${tmp.num }">삭제</a>
							</c:if>
						</td>
					</tr>
				</c:forEach>
